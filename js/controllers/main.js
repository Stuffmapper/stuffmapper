...
function MainController() {
	...
	function initUserData(data) {
		if (data.res.user) {
			$userData.setUserId(data.res.user.id);
			$scope.socket = io('https://www.stuffmapper.com');
	...
	$scope.googleOAuth = function() {
		...
		var w = window.open('https://www.stuffmapper.com/api/v1/account/login/google', '_blank', 'location=no, scrollbars=yes, width=800, height=600, top=' + top + ', left=' + left);
		if(config.ionic.isIonic) {
	...
	$scope.facebookOAuth = function() {
		...
		var left = ((width / 2) - (800 / 2)) + dualScreenLeft;
		var top = ((height / 2) - (600 / 2)) + dualScreenTop;
		var w = window.open('https://www.stuffmapper.com/api/v1/account/login/facebook', '_blank', 'location=no, scrollbars=yes, width=800, height=600, top=' + top + ', left=' + left);
		if(config.ionic.isIonic) {
			w.addEventListener('loadstart', function(event) {
				if (event.url.match('/redirect')) {
					w.close();
